...
async def analyze_chest_xray(
    ...
    if not contents:
        raise HTTPException(
            status_code=400,
            detail="Uploaded X-ray image is empty.",
        )

    if len(contents) > MAX_FILE_SIZE:
        raise HTTPException(
            status_code=400,
            detail="X-ray image must be 10 MB or less.",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="X-ray result could not be saved.",
        ) from exc
...
def get_xray_history(
    ...
    records = (
        ...
        .filter(
            XrayPrediction.user_id
            == current_user.id
        )
        .order_by(
            XrayPrediction.created_at.desc()
        )
        .all()
    )
